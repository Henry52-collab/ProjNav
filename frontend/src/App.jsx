...
const SEVERITY_COLORS = {
  "1–3NM": "#4CAF50", // Green for 1–3NM
  "3–5NM": "#FFEB3B", // Yellow for 3–5NM
  "<1NM": "#FF9800", // Orange for <1NM
  "5+": "#F44336", // Red for 5+NM
};
...
export default function NavCanadaAnalyzer() {
  ...
  const processFile = async () => {
    ...
    try {
      ...
      // Transform the hourly_conflict_counts for proper chart format
      if (analysisResults.insights?.hourly_conflict_counts) {
        const hourlyData =
          analysisResults.insights.hourly_conflict_counts.hours.map(
            (hour, index) => ({
              hour,
              count:
                analysisResults.insights.hourly_conflict_counts.counts[index],
            }),
          );

        // Update the insights with the transformed hourlyData
        analysisResults.insights.hourly_conflict_counts = hourlyData;
      }
      // Transform the severity distribution if it's an object
      if (analysisResults.insights?.severity_distribution) {
        const severityData =
          typeof analysisResults.insights.severity_distribution === "object"
            ? Object.entries(
                analysisResults.insights.severity_distribution,
              ).map(([key, value]) => ({
                name: key,
                value,
              }))
        ...
        // Update the insights with the transformed severity distribution
        analysisResults.insights.severity_distribution = severityData;
      }
      ...
      if (err.message.includes("Failed to fetch")) {
        setError(
          `Cannot connect to backend server at ${API_BASE_URL}. Make sure Flask is running.`,
        );
      } else {
    ...
    >
      ...
      <main className="relative max-w-7xl mx-auto px-6 py-10">
                ...
                <>
                  ...
                  <p className="text-slate-500 mb-6 max-w-md mx-auto">
                    ...
                    <span className="text-xs text-slate-600">
                      Supports Nav Canada flight data format
                    </span>
                  </p>
        ...
        {results && (
          <div className="space-y-6">
              ...
              {expandedSection === "summary" && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-in slide-in-from-top-4 duration-300">
                  ...
                  <StatCard
                    ...
                    value={(
                      results.summary?.total_passengers || 0
                    ).toLocaleString()}
                    color="violet"
                  />
                  <StatCard
            ...
            {results.insights?.hourly_conflict_counts && (
              <section>
                <button
                ...
                {expandedSection === "hourly" && (
                  <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 animate-in slide-in-from-top-4 duration-300">
                    <ResponsiveContainer width="100%" height={350}>
                      <LineChart data={results.insights.hourly_conflict_counts}>
            ...
            {results.insights?.severity_distribution && (
              <section>
                <button
                  ...
                  <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 animate-in slide-in-from-top-4 duration-300">
                    <div className="flex flex-col md:flex-row items-center justify-center gap-8">
                      <ResponsiveContainer width={300} height={300}>
                        <PieChart>
                          <Pie
                            data={results.insights.severity_distribution}
                            cx="50%"
                            cy="50%"
                            innerRadius={60}
                            outerRadius={100}
                            ...
                            dataKey="value"
                          >
                            {results.insights.severity_distribution.map(
                              (entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={
                                    SEVERITY_COLORS[entry.name] || "#6366f1"
                                  }
                                />
                              ),
                            )}
                      ...
                      <div className="space-y-3">
                        {results.insights.severity_distribution.map((item) => (
                          <div
                            key={item.name}
                          ...
                          >
                            <div
                              ...
                              style={{
                                backgroundColor:
                                  SEVERITY_COLORS[item.name] || "#6366f1",
                              }}
                            />
                            ...
                            <span className="text-slate-500">
                              (
                              {(
                                (item.value / results.summary.total_conflicts) *
                                100
                              ).toFixed(1)}
                              %)
                            </span>
              ...
              <section>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-lg">
                      Top Aircraft by Conflict Involvement
                    </span>
                  </div>
                ...
                {expandedSection === "aircraft" && (
                  <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 animate-in slide-in-from-top-4 duration-300">
                    <ResponsiveContainer width="100%" height={350}>
                      <BarChart
                        data={results.charts.top_aircraft}
                        layout="vertical"
                      >
                        <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                        <XAxis
                                  ...
                                  <div className="flex items-center gap-2">
                                    <div className="flex-1 h-2 bg-slate-700 rounded-full overflow-hidden max-w-[100px]">
                                      <div
                                        ...
                                        style={{
                                          width: `${(spot.pressure * 100).toFixed(0)}%`,
                                        }}
                                      />
                                    </div>
                                    <span className="text-sm text-slate-400">
                                      {(spot.pressure * 100).toFixed(1)}%
                                    </span>
                                  </div>
                                </td>
                              </tr>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-slate-500">
                      ({results.tables.flights.length} flights)
                    </span>
                  </div>
              ...
              <section>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-slate-500">
                      ({results.tables.actions.length} actions taken)
                    </span>
                  </div>
                            ...
                            >
                              ...
                              <td className="px-4 py-3 text-sm text-slate-400">
                                {action.action === "delay" &&
                                  action.delay_min && (
                                    <span>
                                      Delayed by {action.delay_min.toFixed(1)}{" "}
                                      minutes
                                    </span>
                                  )}
                                {action.action === "altitude_change" && (
                                  <span>
                                    {action.from_alt}ft → {action.to_alt}ft
                                  </span>
                                )}
                              </td>
                            </tr>
        ...
        {!results && !isProcessing && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 max-w-md mx-auto">
              Upload a JSON file containing flight data to run conflict
              detection analysis.
            </p>
          </div>
        )}
      ...
      <footer className="relative border-t border-slate-800 mt-20">
        <div className="max-w-7xl mx-auto px-6 py-6 text-center text-sm text-slate-600">
          Nav Canada Flight Conflict Analysis System • Powered by Advanced
          Algorithms
        </div>
      </footer>
    </div>
